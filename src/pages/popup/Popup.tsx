...
import LoadingViewSearch from './components/LoadingViewSearch';
import  gSearch  from './function';
...
function Popup() {
  ...
  const [showEngineSearch, setShowEngineSearch] = useState(false);
  const [searchingEngine, setSearchingEngine] = useState(false);
  const [searchedItems, setSearchedItems] = useState<Item[]>([]);
  function fetchItems() {
    setIsLoading(true);
    setShowEngineSearch(false);
    setSearchingEngine(false);
    hideInputAndButton();
 ...
 const searchFromEngine = () => {
  setSearchingEngine(true);
  gSearch(searchedText)
    .then((searchResults) => {
    ...
    <div className="w-[100vw] h-[100vh] bg-black flex flex-col justify-start items-center">
      ...
      {isLoading ? (
        ...
        searchingEngine ? (
          ...
          <SearchResults searchResults={searchedItems} />
        )
      ) : (
